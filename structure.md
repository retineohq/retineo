...
| `registry.ts`     | `Registry`, `SQLiteRegistry`, `OrphanRecord`, `L2Status`                               | SQLite-backed registry: `sources` (`source_id`, `external_id`, `content_hash`, `etag`, `status`, `deleted_at`, `last_seen_at`, `created_at`, `retention_policy`, `sensitivity_level`, `encryption_key_id`), `segments`, `jobs` (lease model), `orphaned_objects`. Implements `RegistryStore`: `get`, `set`, `listByContentHash`, `listBySourceId`. Detects pre-v2 `sources` schema on open and throws rebuild instruction. Convenience methods: `insertSource`, `updateSource`, `deleteSource`, `listSources`, plus jobs/orphans and job queries (`getFailedJobs`, `getL2Status`, `getJobCounts`). Jobs exceeding `maxAttempts` become terminal `FAILED` (re-runnable via `compile`) instead of being silently dropped. |
...
| `embedding-records.ts` | `loadEmbeddingRecords`, `invalidateEmbeddingRecords`, `IndexedEmbeddingRecord`        | Cached JSONL embedding-record loader. Re-reads `embeddings.jsonl` only when mtime/size changes; shared by retrieval and similarity services. |
| `index.ts`         | Barrel export of `hnsw-index.js`, `parquet-store.js`, `embedding-records.js`              | Public embeddings API entrypoint.                                                       |
...
| `similarity-service.ts` | `SimilarityService`, `SimilarOptions`, `SimilarDocument`, `SimilarityServiceDeps`, `createSimilarityService` | Document-level semantic neighbors. Resolves a source document's L3 chunk vectors, queries the shared HNSW index (or brute-force fallback when `mode: 'exact'` or index empty), aggregates chunk scores to a document score, and filters by threshold/ghost status. Read-only; no new artifacts. |
...
| File                         | Exports                                                                                 | Description                                                                                                                  |
| ---------------------------- | --------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `types.ts`                   | `HealthReport`, `MetricResult`, `Finding`, `FindingDocumentRef`, `Recommendation`, `AdvancedMetricHint` | Health analyzer domain types. `Finding.documents` carries `{ contentHash, sourcePath? }` refs resolved from the Registry. `HealthReport` carries `score`, `strong`, `attention`, `recommendations`, `advancedMetrics`. |
| `health-analyzer.ts`         | `HealthAnalyzer`, `DefaultHealthAnalyzer`, `HealthAnalyzerDeps`                         | Orchestrator: reads L0–L3 artifacts from CAS, runs all metrics, builds findings and report. Attaches L2 degradation diagnostics (`l2FailedNodes`, `l2Pending`, `failedJobs`, `l2Status`) and reuses the last report while sources/job state are unchanged. Read-only consumer of pipelines. |
...
| `metrics/orphans.ts`         | `orphans`, `extractTextReferences`, `normalizeReference`                                 | Documents with no links and no inbound references. Connectivity includes `semanticLinks`, parsed markdown text links (`[[wikilinks]]`, `[text](path)`, "см. `file.md`"), Registry child segments, and basename mentions in other documents' L2 summaries. |
...
| `findings-engine.ts`         | `generateFindings`                                                                      | Converts metric results into concrete `Finding` objects. Resolves each `contentHash` to a `sourcePath` via the Registry when available. |
| `report-builder.ts`          | `buildReport`                                                                           | Builds final `HealthReport` JSON. Groups recommendations by finding type and caps the list at 10.                           |
...
| `registry.test.ts`     | Sources CRUD, Segments CRUD + FK cascade, Job lease model (acquire/heartbeat/complete/fail/release, FAILED terminal status after max attempts, `getFailedJobs`, node-level `getL2Status`), Orphan lifecycle | SQLite registry integrity, lease crash recovery, job re-runnability, L2 readiness accounting, orphan purge. |
...
| `l2-generator.test.ts` | Prompt building, JSON validation, control-character sanitization (raw `\n`/`\t` succeeds without retry), retry on invalid JSON/Zod failure, truncation, real provider id/model passthrough, no mock fallback on error, language/conceptsEn output, heuristic fallback | DefaultL2Generator LLM extraction.        |
...
| `similarity-exact-mode.test.ts` | Brute-force `mode: 'exact'` determinism, tie-breaking, empty/unknown hash, ghost filtering, HNSW not built/ensured in exact mode | Exact-mode similarity service. |
...
| `orphans.test.ts`           | Text-reference connectivity: wikilinks, markdown links, bare "см. file.md", inbound links, L2-summary mentions, semanticLinks/child segments. | OrphansMetric edge resolution. |
