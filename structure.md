...
| `retrieval-service.ts` | `RetrievalService`, `DefaultRetrievalService`, `CandidateNode`, `RetrievalResult`, `Citation`, `SearchOptions`, `DocumentHit`, `ChunkHit`, `NavigationNode`, `calculateDocumentScore`, `buildNavigationTree`, `aggregateDocumentHits` | Loads HNSW index at startup; semantic/BM25/hybrid search returns `contentHash` + `chunkHash` + score. `sourcePath`/ghost status resolved post-search via `RegistryStore.listByContentHash`. Ghost candidates return L2 essence only; L0 load is skipped. |
...
| **Clear Registry state during rebuild**                 | `SQLiteRegistry.clearSources/clearJobs/clearOrphans` | `src/storage/registry.ts`                     | `rebuild --force` preserves sources, clears jobs/orphans. |
...
| **Fully rebuild the collection from L0**                | `CLICommands.rebuild`                          | `src/cli/commands.ts`                               | Deletes `index/` + cached L1/L2 and re-queues `GENERATE_L1`→`L2`→`L3` for every source. |
