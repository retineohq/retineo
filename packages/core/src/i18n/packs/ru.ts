...
- Приоритет точным совпадениям перед резюме`,
  ...
  intentPatterns: {
    vague: [
      /^(?:расскажи|объясни|опиши|что такое|как работает|как использует|зачем нужен|о чём)/i,
      /^(?:как|что).+(?:работает|использует|применяет|связано|представляет)/i,
    ],
    precision: [
      /точно/i,
      /точный/i,
    ...
    section: [
      ...
      /глава/i,
      /заголовок/i,
      /в .+ (?:документе|встрече|звонке|файле)/i,
      /о .+ в/i,
      /обсуждали .+ про/i,
    ],
  },
