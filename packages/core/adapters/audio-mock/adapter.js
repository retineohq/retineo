/**
 * ECHO Mock Audio Adapter
 * Generates synthetic transcript with speech blocks and 5-minute segments
...
rl.on('line', async (line) => {
  ...
  switch (req.method) {
    case 'initialize':
      result = { adapterId: 'audio-mock', version: '1.0.0' };
      break;
    case 'capabilities':
      result = { mimeTypes: ['audio/mpeg', 'audio/wav'], extensions: ['.mp3', '.wav'] };
      break;
    case 'ingest':
...
async function ingestFile(uri) {
  ...
  // 1 KB ≈ 1 second
  const durationSec = Math.max(1, Math.floor(stats.size / 1024));
  ...
  for (let segIdx = 0; segIdx < numSegments; segIdx++) {
    ...
    const chunkSize = 20; // words per utterance
    let segContentParts = [];
    ...
    for (let i = 0; i < lines.length; i += chunkSize) {
      ...
      segLocalOffset += lineText.length + 1; // +1 for newline
    }
